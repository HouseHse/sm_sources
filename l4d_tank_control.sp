...
public Plugin:myinfo = 
{
    ...
    author = "arti (added !passtank command by H.se)",
...
public OnPluginStart()
{
    ...
    RegConsoleCmd("sm_passtank", PassTank_Cmd); 
    // Admin commands
...
public Action:PassTank_Cmd(client, args) 
{ 
	if (L4D2Team:GetClientTeam(client) == L4D2Team:L4D2Team_Infected)
	{
		...
		{
			CPrintToChat(client, "{red}This command available only for tank player.");
		}
		else
		{
			PassTankMenu(client); 
			//PrintToChat(client, "DEBUG :  PassTankMenu");
		}
	}
...
PassTankMenu(client) 
{ 
	new Handle:menu = CreateMenu(Select_PassTankMenu); 
	SetMenuTitle(menu, "Select a player who will become tank:\n"); 
	decl String:userid[15], String:name[32]; 
	//new tankClientId;
	//tankClientId = getInfectedPlayerBySteamId(queuedTankSteamId);
	for (new i = 1; i <= MaxClients; i++) 
	{ 
		//if (IsClientInGame(i) && (i != tankClientId) && GetClientTeam(i) == 2 && !IsFakeClient(i)) 
		...
		{ 
			...
			//PrintToChat(client, "DEBUG :  AddMenuItem");
		} 
	} 
	DisplayMenu(menu, client, 0); 
	//PrintToChat(client, "DEBUG :  DisplayMenu");
} 

public Select_PassTankMenu(Handle:menu, MenuAction:action, client, option) 
{ 
	...
	if (target > 0) 
	{
		...
		//PrintToChat(client, "sm_givetank #%s", userid);
	} 
	else CPrintToChat(client, "{olive}This player isn't available anymore."); 
	//PassTankMenu(client); 
}
    ...
    if (tankClientId != -1)
    {
        ...
        if (L4D2Team:GetClientTeam(client) == L4D2Team:L4D2Team_Infected)
        {
            for (new i = 1; i <= MaxClients; i++)
            {
                ...
                {
                    CPrintToChat(i, "{red}%s {default}will become the tank!", tankClientName);
                }
            }
        }
        ...
        {
            CPrintToChat(client, "{red}%s {default}will become the tank!", tankClientName);
        }
    }
...
{    
    ...
    if (! IsFakeClient(tank_index)) 
    {
        for (new i = 1; i <= MaxClients; i++) 
        {
            ...
            CPrintToChat(i, "{red}[Tank Control] {default}(%N) {red}Rage Meter Refilled", tank_index);
        }
...
public outputTankToAll()
{
    ...
    if (tankClientId != -1)
    {
		if (IsVersus())
		{
			...
			{
				CPrintToChatAll("{red}%s {default}will become the tank!", tankClientName);
			}
			else
			{
				PrintToInfected("{red}%s {default}will become the tank!", tankClientName);
				CPrintToChat(tankClientId, "You can swap tank with {olive}!passtank {default}command.");
			}
		}
    }
}
...
bool:IsVersus()
{
	decl String:mpGamemode[32];
	GetConVarString(cvarMpGamemode, mpGamemode, sizeof(mpGamemode));
	if (StrEqual(mpGamemode, "versus") || StrEqual(mpGamemode, "mutation12"))
	{
		return true;
	}
	return false;
